                ...
                for row in reader:
                    if keys.issubset(row.keys()):
                        ...
                        if entry not in duplicate_catcher:
                            ...
                            lesson_term.term1 = Term(value = row["question"], tag = tag1)
                            lesson_term.term2 = Term(value = row["answers"], tag = tag2)
                            lesson.lesson_terms.append(lesson_term)
                            
                            duplicate_catcher.add(entry)
        

    def export_lesson(self, session: Session, lesson_path: str):
        pass